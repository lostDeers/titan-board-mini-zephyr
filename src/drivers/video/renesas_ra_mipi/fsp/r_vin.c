...
/******************************************************************************************************************//**
 * Stop active VIN capture without closing the configured VIN/MIPI CSI instances.
 *
 * This local Zephyr adaptation uses one runtime buffer as both VIN output and UVC
 * payload.  Halt writes as soon as a frame completes so the USB stack never reads
 * from memory that VIN is still updating.
 *
 * @retval   FSP_SUCCESS           Capture was stopped.
 * @retval   FSP_ERR_ASSERTION     p_api_ctrl is NULL.
 * @retval   FSP_ERR_NOT_OPEN      Instance is not open.
 **********************************************************************************************************************/
fsp_err_t R_VIN_CaptureStop (capture_ctrl_t * const p_api_ctrl)
{
    vin_instance_ctrl_t * p_ctrl = (vin_instance_ctrl_t *) p_api_ctrl;
#if VIN_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(p_ctrl);
    ...
    FSP_HARDWARE_REGISTER_WAIT(R_MIPI_CSI->RTST_b.VSRSTS, 0x0);
    R_MIPI_CSI->RXSC_b.RACTDETC = 1;
    R_MIPI_CSI->DLSC0 = R_MIPI_CSI->DLST0 & ~R_MIPI_CSI_DLST0_ULP_Msk;
    R_MIPI_CSI->DLSC1 = R_MIPI_CSI->DLST1 & ~R_MIPI_CSI_DLST0_ULP_Msk;
    uint32_t volatile * p_vc_status = (uint32_t volatile *) &R_MIPI_CSI->VCST0;
    uint32_t volatile * p_vc_clear = (uint32_t volatile *) &R_MIPI_CSI->VCSC0;
    for (uint32_t i = 0; i < 16; i++)
    {
        p_vc_clear[0x10 / 4 * i] = p_vc_status[0x10 / 4 * i];
    }
...
void vin_common_isr (vin_event_t event)
{
    ...
    {
        uint32_t ** buffer_pointer = (uint32_t **) (&R_VIN->MB1);
        args.p_buffer = (uint8_t *) ((R_VIN->MS_b.FBS >= 0x3) ? NULL : *(buffer_pointer + R_VIN->MS_b.FBS)); // Latest valid buffer
    }
